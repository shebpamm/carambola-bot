module.exports.execute = async (client, message, args, guildDocument) => {
  if(args.length === 0) {
    ...
    if(args.length === 1) { //Print out the current value
      if(["role", "mention"].includes(args[0])) { //Check if args[0] is either role or mention
        let currentRole = await message.guild.roles.fetch(guildDocument.config.pugs.pugUserRoleID || ""); //Try to resolve the RoleId found in db.
      ...
      if(args.length === 2) { //Set a new value
