...
module.exports = client => {
  ...
  for (const name in commandFiles) {
    ...
    if(!client.categories[cmd.config.category]) { //If category has not been initialized, initialize.
      ...
      for (const catg of cmd.config.category_aliases) client.category_aliases.set(catg, cmd.config.category);
    }
    ...
    for (const a of cmd.config.command_aliases) client.categories[cmd.config.category].command_aliases.set(a.toLowerCase(), cmd.config.name);
